...
required_libraries = {
    ...
    "cryptography": "cryptography"  # ADDED FOR SECURITY
}
...
from cryptography.fernet import Fernet # IMPORTED FOR SECURITY
import cv2  
import threading
import asyncio
import time
...
# --- SECURITY UPDATE: HIDDEN APPDATA FOLDER ---
# This saves the config in C:\Users\You\AppData\Local\SuperTool_Pro\config.dat
app_data_dir = os.path.join(os.getenv('LOCALAPPDATA'), "SuperTool_Pro")
if not os.path.exists(app_data_dir):
    try: os.makedirs(app_data_dir)
    except: pass

config_file = os.path.join(app_data_dir, "config.dat") 
...
# --- ENCRYPTION KEY ---
# This key encrypts the file so it looks like gibberish if opened.
APP_SECRET_KEY = b'2sC_wJ9yX8x3x2_d7T4a1b0C8k9L5m3N6p8Q1r4S7t2=' 
...
class QueueManagerWindow(ctk.CTkToplevel):
    ...
    # --- WINDOWS FIX: OPEN FILE SAFELY ---
    def open_file(self):
        try:
            os.startfile(self.target_path)
        except:
            os.system(f'explorer "{self.target_path}"')
# ================= FRAMES (SCREENS) =================
...
class LoginFrame(ctk.CTkFrame):
    def __init__(self, app, mode):
        ...
        # --- SECURITY: LOAD ENCRYPTED CONFIG ---
    ...
    def attempt_login(self):
        ...
        # --- FIX: AUTO-DELETE OLD SESSION FILE ---
        # This ensures we start fresh every time
        if os.path.exists("supertool_session.session"):
            try: os.remove("supertool_session.session")
            except: pass
        # -----------------------------------------
        ...
        # --- SECURITY: LOAD & MERGE OLD CONFIG IF EXISTS ---
        old_data = load_secure_config()
        if old_data:
            old_data.update(data)
            data = old_data
            
        save_secure_config(data) # Encrypt and save
        # ---------------------------------------------------
    ...
    def run_async_login(self, api_id, api_hash, bot_token):
        ...
        # --- FIX: USE IN-MEMORY SESSION (None) ---
        # This prevents the app from creating or reading a .session file
        client = TelegramClient(None, api_id, api_hash, loop=loop, connection_retries=10)
        # -----------------------------------------
...
class UploaderFrame(ctk.CTkFrame):
                    ...
                    try:
                        if path.lower().endswith(('.mp4', '.mkv', '.avi')):
                            ...
                            if ret:
                                ...
                                thumb_path = os.path.join(tempfile.gettempdir(), f"thumb_{int(time.time())}_{i}.jpg")
                ...
                if not self.is_cancelled and album_handles:
                    ...
                    try:
                        if len(album_handles) == 1:
                            if album_attrs[0]:
                                await self.app.client.send_file(target_id, album_handles[0], caption=album_captions[0], thumb=album_thumbs[0], attributes=album_attrs[0], supports_streaming=True)
                            else:
                                await self.app.client.send_file(target_id, album_handles[0], caption=album_captions[0], thumb=album_thumbs[0], supports_streaming=True)
                        else:
                            await self.app.client.send_file(target_id, album_handles, caption=album_captions, thumb=album_thumbs, attributes=album_attrs, supports_streaming=True)
                    except Exception as e:
                        err_msg = str(e)
    ...
    # --- WINDOWS FIX: OPEN FOLDER SAFELY ---
    def open_folder(self):
        try:
            os.startfile(self.app.current_download_folder)
        except:
            os.system(f'explorer "{self.app.current_download_folder}"')
...
class SuperToolApp(ctk.CTk):
    def __init__(self):
        ...
        # --- ROBUST TITLE BAR ICON FIX (APPLIED HERE) ---
        icon_path = None
        # 1. Try internal temp folder (For EXE)
        if hasattr(sys, "_MEIPASS"):
            bundled_path = os.path.join(sys._MEIPASS, "logo.ico")
            if os.path.exists(bundled_path):
                icon_path = bundled_path
        
        # 2. Try external folder (For Testing)
        if not icon_path:
            if getattr(sys, 'frozen', False):
                exe_dir = os.path.dirname(sys.executable)
                external_path = os.path.join(exe_dir, "logo.ico")
                if os.path.exists(external_path):
                    icon_path = external_path
    ...
    def show_uploader(self):
        ...
        if self.view_uploader:
            ...
            self.view_uploader = UploaderFrame(self) 
            self.view_uploader.pack(fill="both", expand=True)
    ...
    def show_downloader(self):
        ...
        if self.view_uploader:
            if self.view_uploader.bg_var.get():
                self.view_uploader.pack_forget() 
            else:
                self.view_uploader.destroy()
                self.view_uploader = None
        
        if self.view_downloader:
            self.view_downloader.pack(fill="both", expand=True)
        else:
            self.view_downloader = DashboardFrame(self) 
            self.dashboard = self.view_downloader 
            self.view_downloader.pack(fill="both", expand=True)
